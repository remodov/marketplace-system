...
Сущность контекста Notification. На Tier A — **не DDD-агрегат** (anemic, строка в таблице; логика в сервисе/шедулерах). Контекст-секции (язык, роли, интеграции, события-контракт) — в [корне `notification-service-spec.md`](../notification-service-spec.md).
...
**Не применимо на Tier A.** Сервис событий не публикует (см. [корень → Доменные события](../notification-service-spec.md#5-доменные-события)).
