...
`BR-010`: **Идемпотентность `CreateOrder`.** Заголовок `Idempotency-Key` обязателен; повторный POST с тем же ключом возвращает прежний `orderId`, не создаёт дубль. См. [REST API: заголовки](https://vikulin-va.ru/rest-api/headers/). Нарушение → нет, идемпотентность гарантируется.
