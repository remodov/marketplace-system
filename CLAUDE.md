...
`docs/spec/notification-service-spec.md` — источник правды по сервису (формат — Use Case спецификация Bounded Context; Tier A, агрегатов нет → **один файл**). **Читай в начале сессии** — раннее чтение оседает в тёплом префиксе кэша. Техника (схема БД, стек, топики) — в разделе «Техническая реализация»; домен — в остальных разделах; ссылки между разделами — по именам/якорям.
